...
//        $selfincluded = DB::table('posts')->where('username',  Auth::user()->username)->get();
        ...
        $friends_info_full = DB::table('posts')->select('posts.username as username', 'profileinfo.profileimage as profileimage', 'posts.created_at as created_at', 'posts.description as description', 'posts.id as id', 'posts.imagepath as imagepath', 'posts.views as views', 'post_votes.post_id as post_id', 'post_votes.vote as vote', 'posts.votes as votes', 'posts.comments as comments')->join('profileinfo', 'follows.followsusername', '=', 'profileinfo.username')->join('users', 'follows.followsusername', '=', 'users.username')->join('follows', 'posts.username', '=', 'follows.followsusername')->leftJoin('post_votes', 'posts.id', '=', 'post_votes.post_id')->where('follows.username', Auth::user()->username)->where('post_votes.username', Auth::user()->username)->where('deleted', false)->orderBy('posts.created_at', 'desc')->get(); //'posts.updated_at'
