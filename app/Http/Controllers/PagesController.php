...
//gets all people who follow eachother
    ...
    public function like(Request $request)
    {
        ...
        if(DB::table('post_votes')->where([['username', Auth::user()->username], ['post_id', $request->postid],])->doesntExist()){
            ...
            if((DB::table('post_votes')->select('vote')->where([['username', Auth::user()->username], ['post_id', $request->postid],])->first() == 1) or (DB::table('post_votes')->where([['username', Auth::user()->username], ['post_id', $request->postid],])->first() == -1)){
